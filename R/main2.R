...
# Load data from csv
dataDirectory1 <- '/home/bhet0001/research/readmission/CMS/'
dataDirectory2 <- '/home/bhet0001/research/readmission/Med2Vec/proj/Med2Vec Model/'
claims <- read.csv(paste(dataDirectory2, 'claims_codevectors.csv', sep = ''), header = TRUE)
# 3 patient summary csv are there, chronic disease conditions can change over time
patient_2008 <- read.csv(paste(dataDirectory1, 'DE1_0_2008_Beneficiary_Summary_File_Sample_1.csv', sep = ''), header = TRUE)
patient_2009 <- read.csv(paste(dataDirectory1, 'DE1_0_2009_Beneficiary_Summary_File_Sample_1.csv', sep = ''), header = TRUE)
patient_2010 <- read.csv(paste(dataDirectory1, 'DE1_0_2010_Beneficiary_Summary_File_Sample_1.csv', sep = ''), header = TRUE)
...
# claims <- claims[order(claims$CLM_ADMSN_DT),]
...
df1 <- subset(claims, select = c(PATIENT_ID, year, CLM_ADMSN_DT, NCH_BENE_DSCHRG_DT, 84:284))
...
#df$BENE_BIRTH_DT[is.na(df$BENE_BIRTH_DT)] <- as.Date("1991-04-06")
...
df_new <- subset(df_new, select = -c(PATIENT_ID, year.1))
write.csv(df_new, file = "/home/bhet0001/research/readmission/project/admission_claims_with_med2vec.csv")
